...
namespace GSwap.Services
{
    public class EmailService : IEmailService
    {
        ...
        public async Task<bool> SendEmailAsync(ContactUsAddRequest request)
        {
            //return bool based on response code 202 or 200
            Response response = null;
            bool result = false;
            try
            {
                SendGridClient client = new SendGridClient(@"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");
                SendGridMessage msg = new SendGridMessage();
                msg.AddTo(new EmailAddress("ahpmu925@gmail.com"));
                msg.From = new EmailAddress("ahpmu925@gmail.com");
                msg.Subject = "Contact Us Message From Customer";
                msg.HtmlContent = CreateEmailBody(request);
            ...
            //throw exception based on response message if false
            if (response.StatusCode == System.Net.HttpStatusCode.Accepted)
                {
                    result = true;
                }
        ...
        private static string CreateEmailBody(ContactUsAddRequest request)
        {
            string body;

            using (StreamReader reader = new StreamReader(HttpContext.Current.Server.MapPath(@"~\Content\email_contactUs.html")))
            {
                body = reader.ReadToEnd();
            }
            ...
            body = body.Replace("{phoneNo}", request.PhoneNo);
            body = body.Replace("{city}", request.City);
            body = body.Replace("{address}", request.Address);
            body = body.Replace("{message}", request.Message);

            return body;
        }

        public async Task<bool> Reply (ContactUsReplyRequest request)
        {
            ...
            try
            {
                ...
                msg.From = new EmailAddress("ahpmu925@gmail.com");
                msg.Subject = request.Subject;
                msg.HtmlContent = CreateReplyBody(request);
        ...
        private string CreateReplyBody(ContactUsReplyRequest request)
        {
            string body;

            using (StreamReader reader = new StreamReader(HttpContext.Current.Server.MapPath(@"~\Content\email_replyCustomer.html")))
            {
                body = reader.ReadToEnd();
            }
          
            body = body.Replace("{title}", request.Subject);
            body = body.Replace("{body}", request.Content);
